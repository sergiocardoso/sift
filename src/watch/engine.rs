...
/// every ancestor, not `path` itself) could have been generated by
/// rendering the policy's own template — the same structural question
/// `discover_recursive_dirs_for_date` asks during recursive organize,
/// asked here of one live path instead of a whole tree. `Type` policies
/// have no template, so this is always `false` for them. This is what
/// keeps a recursive Date watch from ever reprocessing its own
/// self-generated destination event (`2026/09/invoice.pdf` right after
/// moving `invoice.pdf` there) into `2026/09/2026/09/invoice.pdf` — no
/// arbitrary sleep, just the same policy object asked the same yes/no
/// question planning already relies on.
fn crosses_strategy_generated_boundary(
    watch_root: &Path,
    path: &Path,
    policy: &EffectivePolicy,
) -> bool {
    let Some(template) = &policy.template else {
        return false;
    };
    ...
    components[..components.len() - 1].iter().any(|c| {
        matches!(
            ...
            std::path::Component::Normal(name)
                if template.component_could_be_generated(0, &name.to_string_lossy())
        )
    })
}
