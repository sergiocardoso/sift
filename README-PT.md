...
Um app pequeno e totalmente separado que mostra um ícone perto do relógio (menu bar no macOS, bandeja no Windows/Linux) listando suas pastas monitoradas — nome, estado (rodando/pausado/parado/com erro de config), com "Abrir pasta", "Pausar"/"Retomar", uma caixa de seleção "Recursive" e "Reapply now" por watch. É uma camada de UI fina sobre a mesma biblioteca `sift` que o CLI usa (`watch::registry::list`, as mesmas funções `cmd_watch_pause`/`cmd_watch_resume` que `sift watch pause`/`resume` chamam) — nunca fala com o daemon do watch diretamente e nunca reimplementa lógica de watch.

Escolher uma pasta no diálogo "Add folder…" é o gesto de autorização deliberada deste app (o mesmo contrato do `--auto-apply` na CLI): diferente do `sift watch add` na CLI — onde a regra do próprio Watch de "nunca varrer arquivos pré-existentes" (veja [Por que o Watch espera antes de mover um novo arquivo](#por-que-o-watch-espera-antes-de-mover-um-novo-arquivo)) significa que uma pasta cheia de arquivos existentes fica intocada até você rodar `sift organize --apply` separadamente — o tray roda uma passada real de `sift organize --apply` no que já existe na pasta *antes* de iniciar o watch, então escolher uma pasta ali já organiza na hora, não só a partir dali em diante.

Mais dois controles por watch que a CLI não expõe diretamente (só acessíveis pelo tray, ou editando o arquivo do registro na mão — ainda não existe subcomando `sift watch` pra nenhum dos dois):

- **Caixa "Recursive"** — liga/desliga o escopo `--recursive` de uma pasta *depois* que ela já está registrada (`sift watch add --recursive` na CLI só define isso uma vez, no momento do registro). Tem efeito ao vivo, no próximo reconcile do daemon rodando — sem precisar pausar/retomar. Recusa ligar sozinho (silenciosamente, igual toda outra ação aqui — olhe o estado da caixa, não um popup) pra uma pasta cuja estratégia ainda não suporta recursão (`audio`/`video`/`photos`/`documents`, veja a seção "Estratégias de organização baseadas em metadados" acima); desligar sempre é permitido.
- **"Reapply now"** — roda uma passada de `sift organize --apply` na pasta imediatamente, usando o escopo `--recursive` que estiver configurado pra ela agora. O mesmo "colocar em dia" manual que você faria com `sift organize <pasta> --apply` num terminal — útil logo depois de jogar uma penca de arquivos na pasta, ou logo depois de ligar o "Recursive", sem esperar o próximo evento do sistema de arquivos.
