...
## [v0.7.8] - 2026-09-13
- Fixed a bug where a nested `.sift.toml` inside a directory whose name matches one of the 9 built-in category names (`Documents`, `Images`, `Video`, `Audio`, `Archives`, `3D`, `Code`, `Data`, `Other`) was silently never honored by `sift organize --recursive` or a running recursive Watch. The reserved-name check that stops recursion from re-entering its own generated output (`Images/Images/...`) fired on the literal directory name alone, with no way to tell "an incidental drop target of the parent's own strategy" apart from "a directory that declares its own governing `.sift.toml`" — so e.g. an `Images/.sift.toml` splitting things further by extension (`*.png -> PNG/`, `*.jpg -> JPG/`, ...) never actually ran once Watch or `--recursive` organize dropped a file into `Images/`. That directory is now still protected when it has no override of its own, but a directory with a valid nested `.sift.toml` is treated as a deliberately governed subtree instead, matching the precedence nested configs already have everywhere else. Fixes the same gap in three places that each independently recognized category directories: `scanner::revalidate_candidate` (Watch's live TOCTOU re-check), `planner::plan_recursive_nested`'s traversal (manual/`--recursive` organize and the tray's "Reapply now"), and `watch::engine::crosses_strategy_generated_boundary` (Watch's own live self-generated-event guard, which now also correctly resolves each ancestor's *own* governing policy instead of checking every ancestor against one flat, leaf-resolved policy).
